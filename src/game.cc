...
Game::renderCell(uint_t x, uint_t y)
{
    ...
    cellRight = cellLeft = NULL;


    if (0 <= x - 1) {
        cellLeft = &CELL_AT(x - 1, y);
    }

    if (x + 1 < w) {
        cellRight = &CELL_AT(x + 1, y);
    }

    const char *cellLeftColour, *cellRightColour;
    cellLeftColour = cellRightColour = bgColour;

    if (cellLeft == NULL) cellLeftColour = "";
    else if (!cellLeft->isRevealed) cellLeftColour = undiscoveredBgColour;

    if (cellRight == NULL) cellRightColour = "";
    else if (!cellRight->isRevealed) cellRightColour = undiscoveredBgColour;

    if (cell.isRevealed) {
        if (!cell.isMine) {
            char cellTxt = nMines == 0 ? ' ' : nMines + '0';

            printf("%s%s", bgColour, colours[nMines]);
            if (cellLeft != NULL) printf(" ");
            printf("%c", cellTxt);
            if (cellRight != NULL) printf(" ");
            ...
            // set background
            printf("%s %s!%s ", cellLeftColour, mineColour, cellRightColour);
        }

    } else if (cell.isFlagged) {
        printf("\x1b[38;2;0;0;0m");
        printf("%s %sF%s ", cellLeftColour, mineColour, cellRightColour);

    } else {
        printf("\x1b[38;2;0;0;0m");
        printf("%s %s?%s ", cellLeftColour, undiscoveredBgColour, cellRightColour);
    }
